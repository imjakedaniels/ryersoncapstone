...
validation$correct <- validation$prediction == validation$churn
print(paste("% of predicted classifications correct", mean(validation$correct)))
...
# an ROC curve.
require(ROCR)
pred <- prediction(validation$probabilities, validation$churn)
...
contacted (α). Implicitly, we are only concerned with the “false positives” represented
by 1-β. 
...
lm(totcalls ~ avgrev, data = final_training_set)
...
lm(avgrev ~ . + . +.,data = final_training_set)

# predict next month's running avg rev
predRev <- predict(lm_model, newdata = validation)
mean(predRev)
```
...
churndata$predNew <- predict(model, type = "response")
data %>% select(churn, predNew) %>% tail()


library(SDMTools)
confMatrixNew <- confusion.matrix(churn, predictions, thresold = 0.5)
...
payoffMatrix$threshold <- data.frame(threshold = seq(from = 0.1, to = 0.5, by 0.1), payoff = NA)
for(i in 1:lnegth(payoffMatrix$threshold)) {
  confMatrix <- confusion.matrix(validation$churn, prediction, threshold = payoffMatrix$threshold[i])
...
#cross validate
split into 4 sets, 1 will be test - use 3 for training and 1 for test

library(boot)
Acc03 <- function(r, pi=0){
  cm <- confuision.matrix(r,pi,threshol=0.3)
