...
library(knitr)
read_chunk('../ryersoncapstone/R - Correcting NA Values.R')
```
...
geom_bar(aes(x=creditcd)) +
  theme_bw()
```
Cleaning all NAs and extreme values, combining all to drop rows easier
```{r}
df1 <- inner_join(Billing, Customer) 
df2 <- inner_join(df1, CustService) 
df3 <- inner_join(df2, Operations) 
df4 <- inner_join(df3, Product) 
tele_clean <- inner_join(df4, Service)
```
...
# change_rev had more instances of NA - opting to drop
na_index <- which(is.na(tele_clean$change_rev))
tele_clean <- tele_clean[-na_index,]

# avg6rev we should have this data, opting to put median value in to keep customers to avoid extreme value's impact on mean
na_index <- which(is.na(tele_clean$avg6rev))
tele_clean[na_index,]$avg6rev <- median(tele_clean$avg6rev, na.rm = T)

# creditcd, converting NA into N (no) and factor making Y/N into 1/0 factor
...
plot(Billing_clean$rev_Mean)
plot(Billing_clean$datovr_Mean)
plot(Billing_clean$change_rev)
...
# same customer information missing, assume they do not have - assigning 0
...
# lor (length of residence in years): - assigning the median
  na_index <- which(is.na(tele_clean$lor))
  tele_clean[na_index,]$lor <- median(tele_clean$lor, na.rm = T)

# adults: keep the na by convert column to factors - anyone with 3-6 adults are combined into one
...
# income (estimated) - convert to Low (1-3), Medium(4-6), High(7-9) - putting NA into Medium(avg)
...
# numbcars NAs: 48854 - half data is missing, NA likely means 0 cars, column is poorly kept so I am dropping it.
tele_clean$numbcars <- NULL

#forgntvlL: let's assume if we dont know forgntvl it is 0 since a majority is 0 already
...
# crdlscod: dropping full column because it has 30+ unique values and is useless for models/reports without any translation
tele_clean$crdlscod <- NULL

# asl_flag: convert to 1,0 factor
tele_clean$asl_flag <- ifelse(tele_clean$asl_flag == "Y", 1, 0) %>% as.factor()

# prizm_social_one: clarifying the text and setting NA to unknown
...
# area: dropping since we don't know where they live
na_index <- which(is.na(tele_clean$area))
tele_clean <- tele_clean[-na_index,]
tele_clean$area <- as.factor(tele_clean$area)

# own_rent:
tele_clean$own_rent <- as.factor(tele_clean$own_rent)

# dwlltype: creating UNKWN column for NA 
...
# marital: - already an UNKNOWN column going to give NA that value
na_index <- which(is.na(tele_clean$marital))
tele_clean[na_index,]$marital <- "U"
tele_clean$marital <- as.factor(tele_clean$marital)

# infobase: two resutls: one option only has 221 inputs the rest are NA values - dropping the variable
tele_clean$infobase <- NULL

# HHStatin - making into factor
tele_clean$HHStatin <- as.factor(tele_clean$HHStatin)

# dwllsize - data is scarce and lacks variation, going to drop variable
tele_clean$dwllsize <- NULL

# forgntvl - let's assume if we dont know forgntvl it is 0 since a majority is 0 already
na_index <- which(is.na(tele_clean$forgntvl))
tele_clean[na_index,]$forgntvl <- 0
tele_clean$forgntvl<- as.factor(tele_clean$forgntvl)

# ethnic (Ethnicity roll-up code) could be valuable for demographics
na_index <- which(is.na(tele_clean$ethnic))
tele_clean[na_index,]$ethnic <- "UNKWN"
tele_clean$ethnic <- as.factor(tele_clean$ethnic)

# kids - U is unknown, Y is Yes so setting all to unknown
sum(which(is.na(tele_clean$kid0_2))) == sum(which(is.na(tele_clean$kid11_15)))
...
Operations (numerics)
```{r}
summary(tele_clean[,42:77])

# avg6mou and avg6qty: we should have this data, opting to put median value in to keep customers to avoid extreme value's impact on mean
...
Correlations
```{r}
# spearman because non-parametrics
nums <- unlist(lapply(tele_clean, is.numeric))  
res <- cor(tele_clean[,nums], method = "spearman")
