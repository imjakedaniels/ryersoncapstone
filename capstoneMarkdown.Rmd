...
CLEANING NA/OUTLIER for Models
...
biscuit <- merge(df1,df2, by = "variable")
names(biscuit) <- c("variable", "pearson", "spearman")
biscuit %>%
  arrange(desc(spearman))

# more positive indicators with spearman
biscuit %>%
  summarise(pearson.postives = length(which(pearson > 0)), spearman.positives = length(which(spearman > 0)))
  
#install.packages('corrplot')
library(corrplot)
corrplot(biscuit, type = "upper", order = "hclust", tl.col = "black")
```
