...
read_chunk('../ryersoncapstone1/R - Correcting NA Values.R')
```
```{r part1}
read_chunk('../ryersoncapstone1/R - Correcting NA Values.R')
```
Combining all tables to make dropping rows easier when cleaning
```{r}
...
truck/rv
```{r}
# same customerinformation missing, assume they do not have - assigning 0
...
adults
```{r}
# keep the na by convert column to factors - anyone with 3-6 adults are combined into one
...
# could be valuable for demographics
...
Extreme Values
```{r}
range(tele_clean$mou_Mean)
plot(tele_clean$mou_Mean)
plot(tele_clean$da_Mean)
plot(tele_clean$ovrmou_Mean)
plot(tele_clean$roam_Mean)
plot(tele_clean$change_mou)
plot(tele_clean$comp_vce_Mean)
plot(tele_clean$comp_data_Mean)
plot(tele_clean$inonemin_Mean)
plot(tele_clean$threeway_Mean)
plot(tele_clean$mou_compvce_Mean)
plot(tele_clean$mou_compdata_Mean)
plot(tele_clean$mou_rvce_Mean)
plot(tele_clean$owylis_vce_Mean)
plot(tele_clean$mou_owylisv_Mean)
plot(tele_clean$iwylis_vce_Mean)
plot(tele_clean$mou_iwylisv_Mean)
plot(tele_clean$peak_vce_Mean)
plot(tele_clean$peak_data_Mean)
plot(tele_clean$mou_peak_vce_Mean)
plot(tele_clean$mou_peak_data_Mean)
plot(tele_clean$opk_vce_Mean)
plot(tele_clean$opk_data_Mean)
plot(tele_clean$mou_opk_vce_Mean)
